...
if (session.direction === 'inbound' && process.env.PSTN_TRUNK_NAME) {
  ...
  const fromHeader = session.sip.headers['From'] || '';
  logger.info({authenticatedUser, fromHeader, direction: session.direction}, 'checking outbound from retell');
  ...
  if (!outboundFromRetell && session.sip_parent_call_sid) {
